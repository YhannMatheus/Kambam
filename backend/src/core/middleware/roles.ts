...
export function requireUserPermission(permission: string) {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      if (!req.user?.userId) {
        res.status(401).json({ error: 'Usuário não autenticado' });
...
export function requireTeamRole(allowedRoles: TeamMemberRole[]) {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      if (!req.user?.userId) {
        res.status(401).json({ error: 'Usuário não autenticado' });
      ...
      const memberRole = teamMember.role as TeamMemberRole;
...
export function requireSupportOrTeamAdmin() {
  return async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const user = await userRepository.findById(req.user.userId);
      if (user && isSupport(user.role as UserRole)) {
        (req as any).userRole = user.role as UserRole;
        next();
        return;
      }
